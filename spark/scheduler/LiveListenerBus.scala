 ...
 * 在start函数被调用之前，所有post的事件都被缓存。只有监听bus开始之后所有的事件才会被传播到相应的监听器。
 */
private[spark] class LiveListenerBus extends SparkListenerBus with Logging {
  ...
  private val eventQueue = new LinkedBlockingQueue[SparkListenerEvent](EVENT_QUEUE_CAPACITY)    //监听事件的队列
